...
class Statistics():
    ...
    def CalculateProfit(self):
            ...
            if idx_order < len(self.stra.Orders):
                ...
                if order.DateTime == self.stra.D[i]:
                    ...
                    # 平仓
                        ...
                        openDire = DirectType.Buy if order.Direction == DirectType.Sell else DirectType.Sell
                        # 找到对应的开仓,计算开平对
                        for x in range(idx_order - 1, -1, -1):  # 第一个-1表示结束只(不包括)
                            openOrder = self.stra.Orders[x]
                            if openOrder.Direction == openDire and openOrder.Offset == OffsetType.Open:
                                profit = (1 if openDire == DirectType.Buy else -1) * (order.Price - openOrder.Price) * order.Volume * self.Product['VolumeTuple']
                                records.append({"多空": "多", "开仓时间": openOrder.DateTime, "开仓价格": openOrder.Price, "平仓时间": order.DateTime, "平仓价格": order.Price, "手数": order.Volume, "净利": profit})
                                tradeTimes += 1
                                if profit > 0:
                                    tradeProfitTimes += 1
        ...
        profit_net = sum([n['净利'] for n in recordsLong]) - sum([n['净利'] for n in recordsShort])
        profit_sum = sum([n['净利'] for n in recordsLong if n['净利'] > 0]) + sum([n['净利'] for n in recordsShort if n['净利'] > 0])
        loss_sum = sum([n['净利'] for n in recordsLong if n['净利'] < 0]) + sum([n['净利'] for n in recordsShort if n['净利'] < 0])
        profit_lots = sum([n['手数'] for n in recordsLong if n['净利'] > 0]) + sum([n['手数'] for n in recordsShort if n['净利'] > 0])
        lots = sum([n['手数'] for n in recordsLong]) + sum([n['手数'] for n in recordsShort])
        profit_max = max(max([n['净利'] for n in recordsLong]), max([n['净利'] for n in recordsShort]))
        loss_max = min(min([n['净利'] for n in recordsLong]), min([n['净利'] for n in recordsShort]))
        a = 0
        b = 0
        max_down = 0
